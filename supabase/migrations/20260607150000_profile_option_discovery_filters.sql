-- Filter columns aligned with profile preference options (intimacy, identity, intent, looking for).

alter table public.user_preferences
  add column if not exists intimacy_roles_filter text[] not null default '{}',
  add column if not exists identity_tags_filter text[] not null default '{}',
  add column if not exists relationship_intent_filter text[] not null default '{}',
  add column if not exists looking_for_filter text[] not null default '{}';
