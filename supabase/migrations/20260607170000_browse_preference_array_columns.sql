...
begin
  ...
  with viewer_preferences as (
    select
      ...
      coalesce(
        nullif(up.intimacy_roles_filter, '{}'::text[]),
        coalesce(up.intimacy_roles, '{}'::text[])
      ) as intimacy_roles_filter,
      coalesce(up.identity_tags_filter, '{}'::text[]) as identity_tags_filter,
      coalesce(
        nullif(up.relationship_intent_filter, '{}'::text[]),
        coalesce(up.relationship_intents, '{}'::text[])
      ) as relationship_intent_filter,
      coalesce(
        nullif(up.looking_for_filter, '{}'::text[]),
        coalesce(up.seeking, '{}'::text[])
      ) as looking_for_filter,
